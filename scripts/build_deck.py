...
SLIDES = [
    ...
    dict(img="mopa_bc_latent_deploy.png", title="Latent-conditioned BC in CAPTURES (the metric that matters)",
         bullets=["Placement-blind BC 0.95 → +latent 0.99 → +oracle 1.11 → expert 1.14 captures/ep",
                  "Knowing the placement (oracle) recovers 85% of the pooled-BC→expert gap — the strategy is VERY valuable for performance",
                  "The unsupervised latent recovers ~22% (within noise today) — honest headroom, gated by the clustering problem"],
         notes="This is why captures beat accuracy: the oracle's value jumps from a barely-visible +2.4 accuracy points to 85% of the performance gap. Be honest — the latent's 22% is within the error bars; the encoder isn't good enough yet, which IS the clustering problem. The oracle 85% is the clear signal and the ceiling."),
    dict(img="mopa_bc_latent_sweep.png", title="Why the latent should help: encoder quality is the lever (accuracy)",
         bullets=["Give the encoder more observation: the VAE latent's probe climbs 0.60 → 0.78",
                  "Action accuracy tracks it — overtakes vanilla by 50 steps, reaches the oracle by 125",
                  "JEPA's probe stays flat on occupancy → no gain (winner depends on the feature)"],
         notes="Supporting view, action accuracy, per-episode latent: the BC gain tracks the probe almost exactly, so a better encoder means more gain. Caveat to say: the placement is FIXED, so 'more observation' = a better occupancy estimate (averaging), not the strategy unfolding."),
