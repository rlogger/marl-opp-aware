"""
Two-team independent IQL for MPE simple_tag, MLP variant (no RNN).

Structural twin of iql_teams.py but swaps the RNN Q-network for a feedforward
MLP. Motivation: simple_tag is fully observable (each agent's obs already
includes relative positions of all other entities), so the RNN's memory is
unnecessary and only adds wall-clock cost. Dropping it also simplifies the
tree-expanded planning work in Exp 3 -- no hidden state to carry through the
lookahead simulator.

Run:
    python src/iql_teams_mlp.py +alg=ql_teams_mlp_simple_tag
"""
import os
import copy
import time
        ...
        def _update_step(runner_state, _):
            ...
            def _step_env(carry, _):
                ...
                def team_act(team, rng_a):
                    ags = teams[team]
                    _obs = batchify_team(last_obs, ags)  # (team_n, NUM_ENVS, obs_dim)
                    q = jax.vmap(networks[team].apply, in_axes=(None, 0))(
            ...
            def _learn_team(team, train_state, minibatch):
                ags = teams[team]
                _obs = batchify_team(minibatch.obs, ags)   # (team_n, B, T, obs_dim)
                ...
                def _loss_fn(params):
                    q = jax.vmap(networks[team].apply, in_axes=(None, 0))(params, _obs)
                ...
                (loss, qv), grads = jax.value_and_grad(_loss_fn, has_aux=True)(train_state.params)
                train_state = train_state.apply_gradients(grads=grads)
                train_state = train_state.replace(grad_steps=train_state.grad_steps + 1)
                return train_state, (loss, qv)
            ...
            def _learn_phase(carry, _):
                ...
                losses = {}
                for t in team_names:
                    ns, (loss, qv) = _learn_team(t, train_states[t], minibatch)
                    new_states[t] = ns
                    losses[t] = (loss, qv)
            ...
            (train_states, rng), losses = jax.lax.cond(
                ...
                lambda ts, r: (
                    (ts, r),
                    {t: (jnp.zeros(config["NUM_EPOCHS"]), jnp.zeros(config["NUM_EPOCHS"]))
                     for t in team_names},
                ),
                train_states, _rng,
            )
...
def single_run(config):
    ...
    alg_name = config.get("ALG_NAME", "iql_teams_mlp")
