...
faced with the unseen prey. We give the formal setup, the encoder objective, and
the metric definitions, and discuss the regime-dependence of each result.
\end{abstract}
...
%% ==================================================================
\section{From preconditions to the method}

With P1--P3 established, the proposal's two parts are directly actionable on this
task. \textbf{Part 1:} replace the occupancy-VAE diagnostic with the full
latent-conditioned opponent model $\pi^{\mathrm{red}}_\psi(a \mid s, \zlat)$,
trained by behavior cloning (then contrastive preference learning) against the
specialist trajectories, with $\zlat$ from the encoder validated here.
\textbf{Part 2:} embed $\pi^{\mathrm{red}}_\psi(\cdot \mid s, \zlat)$ as the
red-action sampler in a MuZero-style search and condition the blue value function
on $\zlat$; P3 gives the concrete target, recovering on the OOD diagonal the
captures a co-trained predator loses. The headline numbers to beat are in
Table~\ref{tab:p3}: an opponent-aware circle-predator should catch the
corners-prey at closer to $1.29$ than the unaware $0.72$.
